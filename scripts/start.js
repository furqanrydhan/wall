...
function addMiddleware(devServer) {
  ...
  devServer.use(historyApiFallback({
    ...
    disableDotRule: true,
    // For single page apps, we generally want to fallback to /index.ejs.
    // However we also want to respect `proxy` for API calls.
    // So if `proxy` is specified, we need to decide which fallback to use.
    // We use a heuristic: if request `accept`s text/html, we pick /index.ejs.
  ...
  if (proxy) {
    ...
    // - /index.ejs (served as HTML5 history API fallback)
  ...
  // It may be /index.ejs, so let the dev server try serving it again.
...
function runDevServer(port) {
  ...
  // Our custom middleware proxies requests to /index.ejs or a remote API.
